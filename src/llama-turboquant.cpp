...
bool require_f32_tensor(
    ...
    if (expected_shape.size() > GGML_MAX_DIMS ||
        (expected_shape.size() < GGML_MAX_DIMS && ne[expected_shape.size()] != 1)) {
        return set_error(error, "GGUF TurboQuant tensor `" + name + "` has an invalid rank");
    }
    for (size_t dim = 0; dim < expected_shape.size(); ++dim) {
        if (ne[dim] != expected_shape[dim]) {
...
bool require_rotation_tensor(
    ...
    const int64_t rows = ne[0];
    const int64_t columns = ne[1];
